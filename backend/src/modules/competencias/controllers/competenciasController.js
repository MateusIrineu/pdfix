...
class CompetenciaController {
  ...
  static async listarCompetencias(req, res) {
    try {
      ...
      if (listarCompetencias.length === 0) {
        return res
          .status(200)
          .json({ mensagem: "Nenhuma competência encontrada." });
      }
  ...
  static async atualizarCompetencia(req, res) {
    try {
      ...
      if (!buscarId) {
        return res
          .status(400)
          .json({ mensagem: "ID da competência é obrigatório." });
      }
      const {
        usuario_id,
        nome_competencia,
  ...
  static async deletarCompetencias(req, res) {
    try {
      await competenciasModel.truncate();
      return res
        .status(200)
        .json({ mensagem: "Competência deletada com sucesso!" });
    } catch (error) {
      return res.status(500).json({
